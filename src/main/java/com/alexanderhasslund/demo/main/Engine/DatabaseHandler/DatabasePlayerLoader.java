...
public class DatabasePlayerLoader {
    ...
    public List<Player> updatePlayerListInventory(List<Player> playerList) {
        ...
        try (Connection connection = DatabaseConnector.getConnection()) {
            String getActivePlayerData = "select i.ItemName , i.ItemDamage, i.ItemInitiative, i .ItemLevelLock , i.ItemDefence, i.ItemBlock, p.ItemSlot, max(p.RegistrationDate) as regDate from dungeonrun.playerinventory p \n" +
                    ...
                    "group by i.ItemName , i.ItemDamage, i.ItemInitiative, i .ItemLevelLock , i.ItemDefence, i.ItemBlock, p.ItemSlot order by regDate asc";
            try (PreparedStatement statement = connection.prepareStatement(getActivePlayerData)) {
